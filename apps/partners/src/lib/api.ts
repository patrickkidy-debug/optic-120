...
api.interceptors.request.use((config) => {
  const token = usePartnerAuthStore.getState().accessToken;
  if (token) {
...
export async function refreshPartnerSession(): Promise<string | null> {
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const res = await axios.post(
        `${API_URL}/partners/auth/refresh`,
        {},
        { withCredentials: true, timeout: 60000 },
      );
      const { accessToken, partner } = res.data;
      usePartnerAuthStore.getState().setAuth(accessToken, partner);
      return accessToken;
    } catch (err) {
      ...
      usePartnerAuthStore.getState().clear();
      return null;
    }
  }
  usePartnerAuthStore.getState().clear();
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (status === 401 && original && !original._retry && !url.includes('/partners/auth/')) {
      original._retry = true;
      if (!refreshing) {
        refreshing = refreshPartnerSession().finally(() => {
          refreshing = null;
        });
      }
